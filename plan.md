...
Last updated: Phase 0 (1:1 chat delivery fix after sessions success: added realChatUnsubsRef + currentActiveChatRef, made bg+active 1:1 listeners robust (dedup, cleanup stale like group), load now pushes to realChatMessages for open chat, handlers log live triggers, send forces server refresh. Mirrors the working group chat pattern. Pushed after build. User to hard refresh + test send from A, receive live on B's list + open chat on different device/browser without manual update. **PHASE 0 SIGN-OFF READY** (continua).
