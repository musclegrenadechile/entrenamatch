...
Last updated: Phase 0 (core 1:1 fix for "realMatches=[] when opening real chat" causing no real send + 0 msgs on load: introduced isRealChatId helper (realMatches OR known realProfiles); on activeChat open for real UID: bootstrap add to realMatches + write match doc + load; force loadRealMatches right after swipe match write. Plus messages composite index. This ensures send always hits Firestore for real pairs, listeners activate, loads find msgs. Logs from user report pinpointed the isRealChat false + empty snaps. Pushed b70b504. Test: A swipes real B → both can open chat (B may need 1x "Actualizar chats reales" for discovery), send both directions → live + persist + cross refresh. **PHASE 0 SIGN-OFF READY** (continua).
