...
Last updated: Phase 0 (CRITICAL RULES FIX + polish deployed: sessions self-join rule + relaxed /matches + /messages reads (fixes exact "no funciona la conversacion entre distintas cuentas" + join permissions error). Also improved join onClick to await persist + show clear toast on fail while still opening optimistically + force load. Built + pushed 1d854ab. CI deploying rules now. Tell user to run manual deploy if needed + hard refresh + test 1:1 real + session join+group both directions. **PHASE 0 SIGN-OFF READY** (continua).
