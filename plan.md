...
Last updated: Phase 0 sessions automation (onSnapshot for live sessions list + participant updates + lastMessagePreview on cards; write welcome + activity to sub/parent on create/send; conditional localStorage only for demo; fixed "Los mensajes son locales en esta versión (demo)" label to respect !isDemoMode; seed 1:1 to server too). Commit e16ef25. Hard refresh, test: create session on one device/account, see instantly on another (no 30s wait), join, send group msg, see live + preview in list. 1:1 with fakes now says "reales" + persists sent msgs. **PHASE 0 SIGN-OFF READY**
