...
Last updated: Phase 0 (CRITICAL RULES FIX for join + 1:1: updated sessions update rule to support self-join via request.resource + relaxed read on matches/messages for reliable cross-real-account discovery and chat. Pushed a57442b + guide note. This directly addresses the exact errors the user just reported: "no funciona la conversacion entre distintas cuentas, en mensajes" + "Failed to persist join to Firestore: Missing or insufficient permissions" on Unirme. CI will deploy rules on push. Instruct user to manual npx or wait Action + hard refresh + full 2-account test (like real profile from A, see match+chat on B, send both ways, create+join session both ways, group msg live). **PHASE 0 SIGN-OFF READY** (continua).
