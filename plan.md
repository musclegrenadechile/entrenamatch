...
- **Global Feed Tab enhanced (spectacular, full green light)**: 'feed' tab with sticky header, owner avatars/photos, REAL badges, "Publicar" quick button (goes to profile), delete own posts inline, like pop, full comment previews that open modal, "Ver perfil completo", "Cargar más". Collects from all loaded community posts (sorted newest), shows up to 30 + load more. Auto + manual refresh. Integrated with live updates via shared state. Empty state with CTAs. Owner photos from realProfiles. + Pinned posts: 📌 in own/feed for own posts (pinned first in global sort, badge shown). + Pinned-only filter toggle in feed header. + Pinned highlighted in explore/matches teasers + full profile preview. + Added search bar + "Solo reales" toggle + filter count in feed header for more control. More glass/motion polish on cards. Makes global muro feel premium and alive. Sigue con todo! :D
- Delete own comments: × in previews and full modal for your comments, removes from FS/array.
- More glass/motion: whileHover scale on post cards in own and full profile views.
