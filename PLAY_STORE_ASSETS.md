...
**Current version for this upload**: versionCode 13 / 0.1.10-prealpha — **JSON FIXED + FULL RELEASE WITH ALL FIXES**: google-services.json (the correct one with com.entrenamatch.app) placed from Downloads by user → AI copied to android/app/, full pipeline re-run successfully (now logs "CONTAINS MATCHING package" and build succeeds with proper FCM/Push/Integrity). All recent improvements included: EntrenaSync join now attractive (loading states on buttons, auto-nav to rich sync panel, no spam), direct attractive Publish modal from Feed (no disappointing redirect to profile + nice composer with real file picker), muro photo UX completely overhauled (no more ugly URL prompt, proper native file picker + great preview), live visibility fixed (people with trainingNow now appear even without location, global poller + orderBy), nivel headers in profile now square/consistent chips + grid fixed to 3-col for uniform cards, Feed header filters (Live/Fijados) now properly horizontally scrollable, many visual/iconic muro upgrades. New AAB produced with correct Firebase config. Version bumped across all files. See BETA/plan for test protocol. Error TDZ solucionado hace tiempo. JSON fixed → full working closed testing build ready.
