...
**Current version for this upload**: versionCode 12 / 0.1.9-prealpha — **LA MÁS TOP UPDATE DE TODAS + DISRUPTIVA + FULL PIPELINE RUN ("hagas todo")**: Feed 2.0 ultra atractivo (header glass+blur premium + gradient chips + big +Publicar CTA, live teaser row con stagger, post cards con photo hover-scale + overlay + lightbox modal, stagger anims, quick emoji reactions 🔥💪❤️👏 con pop counts, richer comments + pinned, empty state espectacular). + Reacciones en muro propio. **NUEVO KILLER ÚNICO Y DISRUPTIVO**: EntrenaSync - la primera experiencia de "entrenamiento sincronizado real-time" del mercado: únete al live de alguien y arranca timer compartido + acciones en vivo (buena forma, series...) que se postean auto a ambos muros creando accountability instantánea aunque no estés en el mismo gym. Ahora persistente (FS syncActions/syncStreak), mirror en loads (botón "Refrescar" para pull inmediato), stats Syncs, +1 streak al terminar, **rating final 1-5 estrellas** que boostea más. Promoción en live modal. FOMO + loops de retención brutales + data moat para matching. **Fix TDZ/JSX errors**: resuelto (mirror effect movido después de states, rating sin duplicados, modal nesting limpio). **FULL "HAGAS TODO"**: google-services.json from the exact path you indicated (Downloads) was located + copied to android/app/ by AI, full release pipeline executed end-to-end (npm run android:build with CAPACITOR + gradle clean bundleRelease with JAVA/ANDROID_HOME from publish scripts). **Build now HARD FAILS with exact actionable error** ("No matching client found for package name 'com.entrenamatch.app' in google-services.json") + loud log from our new strict matcher in build.gradle:64 — this explains the Play Store launch crash (wrong package in the json you pointed = Firebase/FCM not inited for com.entrenamatch.app). New AAB not produced until correct json. See updated BETA + plan for exact Firebase steps to add "com.entrenamatch.app" client and re-download. Once you place the right json (overwrite android/app/), re-run build-aab-now.bat or publish-play.ps1 and AI can finish the AAB + upload prep. Version bumped, stricter checks, all EntrenaSync/Feed/live polish included. Error TDZ solucionado.
