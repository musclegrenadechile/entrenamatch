...
- AAB ready: EntrenaMatch-v0.1.14-live-map.aab (and EntrenaMatch-release.aab) in root. Also fresh debug APK (EntrenaMatch-debug-tdz-fix.apk) with real-time live training map (Leaflet + real geo) + TDZ/hoisting fix for the map code (moved hooks to correct declaration order). Map shows pins by zones, sized by activity, live updates, popups with join + distance. Test with multiple live users in different cities. Toggle "Mapa en tiempo real" in Explore.
