...
- **El error #310 de React en el Feed ya está arreglado** (useMemo feedComputation levantado a top-level).
- **NUEVO (jun 2026): error #310 "al hacer login" (index-*.js useEffect en App)**: Aparecía exactamente al transicionar de pantalla de login a la app (minified "Rendered more hooks than during the previous render"). Causa: durante el trabajo del mapa en vivo, el useEffect de Leaflet (showLiveMap + liveTrainingNow + markers) quedó declarado en el fuente DESPUÉS de los `if (!isAuthenticated) return <AuthScreen/>` y el check de onboarding. En renders sin usuario se ejecutaban solo los hooks previos al guard y se retornaba; al loguear exitosamente se ejecutaban hooks extra → mismatch de conteo de hooks para React.
  - Fix aplicado: movido el useEffect completo del mapa a una posición estable ANTES de cualquier retorno condicional (justo después del useMemo de liveTrainingNow y su useEffect de notifs de urgencia, pero antes del gate de isAuthenticated). Ahora el conteo y orden de hooks es idéntico siempre.
...
- **Mapa en vivo "sigue con todo" ultra pulido (precioso + único)**: (v0.1.20+)
  - Marcadores foto/iniciales con pulso vivo, radar sweep animado, filtro cerca de mí, círculo radio personal, leyenda zonas, Centrar, persistencia preferencia, hápticos, integración header LIVE pill, empty states premium.
  - **Toque disruptivo extra**: líneas tether ligeras (discontinuas naranja) entre pares que están en Sync ahora mismo. Se ven en el mapa como "rituales conectados". Popup explica. Único y visualmente mágico.
  - **FIX**: el mapa y botones (toggle "Ver mapa por zonas", Centrar, filtro cerca, overlays) ahora responden a presión/tap. Agregado pointer-events-none a backgrounds decorativos, z-index altos a overlays y sección, invalidateSize() después de mostrar/toggle (problema común en WebView), timeout popup aumentado, touch-action CSS, removido handler click conflictivo en markers (ahora popup maneja join/perfil). Panning, taps en markers, popups y botones funcionan en Android. Prueba presionando el botón del mapa, el mapa mismo, markers y botones Centrar/filtro.
  - Legend ahora interactivo: toca una zona (Viña, Stgo, etc.) para filtrar el mapa solo a esa ciudad. Toca de nuevo o "✕ todas" para reset. Perfecto para "sigue con todo el mapa". Los markers, tethers y self se actualizan al instante.
- **Geo crash fix (v0.1.21+)**: activar geolocalización (botón GPS en Perfil o al activar live) ya no crashea ni pone pantalla en negro. Guard anti-concurrent requests, fallback accuracy (highAcc primero, si falla low), guard !currentUser antes de save (evita partial user que crasheaba renders), más robustez en catch/finally. Prueba: presiona "📍 Actualizar ubicación real (GPS)" o toggle live -> debe pedir permiso GPS, obtener coords sin crash, setear distancias y self marker en mapa.
...
- **La app falla / se cierra justo al abrir (en APK descargada de Play closed)** → Causa: AABs built with google-services.json that has no client for 'com.entrenamatch.app' (e.g. the one from "esa direccion"/Downloads only lists com.muscle.entrenamatch + _nuevo; or completely missing). Firebase native (for @capacitor/push-notifications + play-integrity) can't init → crash on cold start.
  ...
  - **Solución para el equipo / AI**: In Firebase Console (entrenamatch project) → Project settings → Your apps → Add Android app with **exact package com.entrenamatch.app** → download the json → overwrite `entrenamatch\android\app\google-services.json` → say "json fixed, build the AAB" or "sigue con todo". AI will re-run the full build (now will succeed), copy EntrenaMatch-v0.1.9-....aab , update docs, and prep the Play upload. Then upload the good AAB to closed (first time often manual to seed the track). 
...
- Version bumped to 19 / 0.1.14-live-map. Real-time live training map (Leaflet + real geo) is the main new feature.
...
- Test focus for this build:
  ...
  10. Test the new real-time map: In Explore, toggle "Ver mapa por zonas". Should show Leaflet map with colored circle markers for live users (sized by joins, colored by city/zone). Zoom/pan, click marker for popup with name, time left, distance, join button. Updates live as people toggle trainingNow. Works best with real GPS + users in different cities (Viña, Santiago, etc.). If no location, shows prompt.
...
- Fresh debug APK with the fix: `EntrenaMatch-debug-storage-fix-0.1.39-20260604-2053.apk` (or the latest generic `EntrenaMatch-debug.apk`). This one includes the graceful fallback so you can still use the beautiful new "Capturar momento icónico" flow.
...
**IMPORTANTE para fotos reales (Storage 403 unauthorized):**
Si ves el error `storage/unauthorized` o 403 al subir foto en el muro (path `posts/{tu-uid}/...`):
- Ya agregué `storage.rules` (solo tú puedes escribir en tu carpeta de posts, cualquiera autenticado puede leer las imágenes para que aparezcan en feeds/muros ajenos).
- Actualicé `firebase.json` para incluir las rules.
- En el código ahora hay fallback robusto: si falla el upload a Storage, igual te deja poner la foto (embebida) y publicar, con toast claro.
- Para arreglarlo de verdad (uploads https livianos): en la raíz del proyecto corre:
  ```
  npx firebase deploy --only storage --project entrenamatch
  ```
  (primero `firebase login` si no estás autenticado en CLI).
- Después del deploy, los uploads de muro/feed/arena deberían ir directo a Storage sin 403.
- Mientras, las fotos "icónicas" que subas se guardan embebidas (como en las versiones antiguas), pero el flujo UI nuevo funciona 100%.

 
 # #   N O T A   D E   V I S I � N   ( a c t u a l i z a d o   e n   b u r s t   s i g u e   a   m i l ) 
 
 L a   a p p   a h o r a   e s   c o n s i s t e n t e m e n t e   p o s i c i o n a d a   c o m o   l a   p r i m e r a   r e d   s o c i a l   r e a l   d e l   f i t n e s s   d e   a l t o   r e n d i m i e n t o : 
 
 -   T u   * * R e d   d e   E n t r e n a S y n c * *   ( c o n   N e t w o r k   P o w e r   y   v a l o r   p o r   a l i a n z a )   e s   t u   g r a f o   s o c i a l   y   c a p i t a l   d e   r e n d i m i e n t o . 
 -   E l   * * P u l s o   G l o b a l * *   ( m a p a )   e s   e l   f e e d   v i s u a l   e n   t i e m p o   r e a l   d e   d � n d e   e s t �   p a s a n d o   e l   e s f u e r z o   s i n c r o n i z a d o   d e   l a   r e d . 
 -   L o s   * * H i g h l i g h t s   d e   S y n c * *   s e   p r o p a g a n   e n   e l   f e e d   c o m o   c o n t e n i d o   q u e   c o n s t r u y e   c u l t u r a   y   e s t a t u s . 
 -   L o s   p o s t s   d e   s y n c   s o n   c u a n t i f i c a d o s   c o n   i m p a c t o   e n   l a   r e d   ( ' + X X   m i n   d e   p r o g r e s o   c o m p a r t i d o   q u e   f o r t a l e c e   n u e s t r a   r e d ' ) . 
 -   L e n g u a j e   c o h e r e n t e :   ' r e d ' ,   ' p o d e r ' ,   ' c a p i t a l ' ,   ' g r a f o ' ,   ' p u l s o ' ,   ' a l i a n z a s   d e   a l t o   r e n d i m i e n t o '   -   s e r i o ,   a m b i c i o s o ,   � p i c o   s i n   f a n t a s � a . 
 
 E s t o   m a n t i e n e   l o s   5   m e c � n i c o s   i n t a c t o s   p e r o   l o s   h a c e   s e n t i r   c o m o   l a   i n f r a e s t r u c t u r a   d e   c a t e g o r � a   q u e   n a d i e   h a   c r e a d o :   e l   s o c i a l   g r a p h   p a r a   e n t r e n a m i e n t o   s i n c r o n i z a d o . 
 
 A c t u a l i z a d o   e n   c � d i g o   ( A p p . t s x ,   i n d e x . c s s ) ,   l a n d i n g ,   r o a d m a p . 
 
...
 
 L a   a p p   a h o r a   e s   c o h e r e n t e m e n t e   l a   i n f r a e s t r u c t u r a   d e   l a   p r i m e r a   r e d   s o c i a l   r e a l   d e l   f i t n e s s   d e   a l t o   r e n d i m i e n t o : 
