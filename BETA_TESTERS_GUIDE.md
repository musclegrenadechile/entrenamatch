...
**1:1 messaging fully working (2025-04)**: Sesiones + group + 1:1 real entre cuentas live y cross-device. UI declutter + premium polish para elegir perfiles (lastSync added to recs header, calmer texts, empty states with sync, 'en vivo' badges, filters cleaned, spinners, lastSync in multiple headers/empties/Profile). lastSync "hace Xs" + Actualizar visibles. Hard refresh después de pushes.
