...
- Profile ultra vivo y atractivo: live pulsing hero + "ENTRENANDO AHORA" banner + real GPS indicator + "📍 Actualizar ubicación real" button, "Mi vida de entrenamiento" summary, legends with progress bars + LV badges, recent activity feed from muro likes/comments, gallery quick ★ main photo, richer live stats, Vibe Score composite + Logros achievements row (1d streak, 7d host, 1st legend, etc.). Real geo makes distances accurate everywhere (explore, arena header, live cards with "CERCA" badges).
...
- AAB ready: EntrenaMatch-v0.1.13-geo-arena.aab (and EntrenaMatch-release.aab) in root. Also fresh debug APK (EntrenaMatch-debug-sin-parar.apk) with latest polishes (profile vibe/achievements, geo badges, etc.).
