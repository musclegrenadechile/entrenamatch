...
- FCM / Push: Client + token save to /userPushTokens started (for server sends). On APK closed: login real -> console [FCM] token saved. (Full background needs google-services.json in android/app/ + server Cloud Function or manual test via Firebase Console using the token from Firestore /userPushTokens/{uid}). Test: use Firebase Console (Engage > Messaging > New campaign or Send test) to send to the exact token saved. Update APK (with google-services if placed) to see real notifs while closed/background. Sigue con todo: docs + BETA protocol ready; next build will include google-services for full native push.
