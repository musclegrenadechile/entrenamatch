...
**Último fix de rules + listeners (2025-04)**: Sesiones join+group ya funcionan. Para 1:1: se agregaron ref-managed bg listeners (igual que group chats), direct logs "📨 Live 1:1", loadRealChatMessages ahora también refresca realChatMessages cuando el chat está abierto (vía currentActiveChatRef), active + bg handlers ahora loguean cuando llega snapshot, send fuerza refresh propio, y setup más robusto sin races. Esto asegura que cuando uno envía, el listener del otro lado dispara load y actualiza la vista (lista previews + chat abierto). Hard refresh después de deploy.
