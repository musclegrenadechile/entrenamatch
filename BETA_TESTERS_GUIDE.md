...
- **La app falla / se cierra justo al abrir (en APK descargada de Play closed)** → Causa: los AABs hasta 0.1.5 se compilaron SIN `android/app/google-services.json`. El plugin de notificaciones push requiere que Gradle aplique el plugin de google-services para inicializar Firebase nativo correctamente. Sin él, FirebaseMessaging / init falla en el arranque nativo y mata el proceso Android. 
  - **Solución para el equipo**: Coloca el google-services.json correcto (Firebase Console → tu proyecto → app Android com.entrenamatch.app) en `android/app/`, rebuild (ahora versionCode 9 / 0.1.6-prealpha con chequeo mejorado en gradle), sube **nuevo release** al track cerrado. 
