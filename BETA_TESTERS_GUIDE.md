...
**1:1 messaging fully working (2025-04)**: Sesiones + group + 1:1 real entre cuentas live y cross-device. UI declutter + premium polish para elegir perfiles (inline lastSync next to Actualizar in Explore header + to recs, calmer texts, empty states with sync info, 'en vivo' badges, filters cleaned, spinners, lastSyncs across headers/empties/Profile). lastSync "hace Xs" + Actualizar visibles. Hard refresh después de pushes.
