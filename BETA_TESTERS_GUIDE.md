...
**1:1 messaging fully working (2025-04)**: Sesiones + group + 1:1 real entre cuentas live y cross-device. UI declutter + premium polish para elegir perfiles ('en vivo' badge in 'Tus matches' header, 'Ver X disponibles' in filters, 'en vivo' in recs, 'disponibles ahora', inline lastSync next to Actualizar (JSX clean), calmer texts, empty states with sync, filters cleaned, spinners, lastSyncs). lastSync "hace Xs" + Actualizar visibles. Hard refresh después de pushes.
