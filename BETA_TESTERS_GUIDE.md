...
**1:1 messaging fully working (2025-04)**: Sesiones + group + 1:1 real entre cuentas live y cross-device. UI declutter + premium polish para elegir perfiles ('en vivo' badge to real profiles count in Explore header, 'en vivo' in 'Tus matches', 'Ver X disponibles', 'en vivo' in recs, 'disponibles ahora', inline lastSync, softer empty texts, updated real-testers welcome (highlights polish + Feedback), client push setup, calmer texts, empty states with sync, filters cleaned, spinners, lastSyncs). Privacy/Términos full + linked in Profile. lastSync "hace Xs" + Actualizar visibles. Hard refresh después de pushes.
