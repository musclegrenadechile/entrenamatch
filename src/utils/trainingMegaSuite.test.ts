...
describe('trainingMegaSuite', () => {
  it('mega-inventario 7 bloques oleadas 361–425', () => {
    expect(countTrainingMegaBlocks()).toBe(7)
    expect(trainingFullMegaRange()).toEqual({ from: 361, to: 425 })
    expect(countTrainingMegaOleadas()).toBe(65)
  ...
  it('trainingMegaBlockById', () => {
    ...
    expect(fuelPlan?.range).toEqual({ from: 411, to: 425 })
    const postFull = trainingMegaBlockById('polish-post-full')
    expect(postFull?.range).toEqual({ from: 421, to: 425 })
    expect(postFull?.closedOleada).toBe(425)
  ...
  it('cierre post-full 421–425 (oleada 425)', () => {
    expect(areAllTrainingMegaSubBlocksClosed(425)).toBe(true)
    expect(isTrainingMegaFullyClosed(425)).toBe(true)
  })
})