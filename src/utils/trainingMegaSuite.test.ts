...
describe('trainingMegaSuite', () => {
  it('mega-inventario 4 bloques oleadas 361–402', () => {
    expect(countTrainingMegaBlocks()).toBe(4)
    expect(trainingFullMegaRange()).toEqual({ from: 361, to: 402 })
    expect(countTrainingMegaOleadas()).toBe(42)
  ...
  it('trainingMegaBlockById', () => {
    ...
    expect(plan?.range).toEqual({ from: 401, to: 402 })
  })
})